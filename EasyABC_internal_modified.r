...
.move_particle <- function(param_picked, varcov_matrix) {
  # library(mnormt)
  mnormt::rmnorm(n = 1, mean = param_picked, varcov = varcov_matrix)
}
...
.selec_simul_alpha <- function(summary_stat_target, param, simul, sd_simul, alpha, dist_weights) {
  ...
  remove = simul[is.na(dist)|dist>tol]
  remove = remove[(length(remove)/2+1):length(remove)]
  unlink(paste0("sim_final",remove,".csv"))
  unlink(paste0("residuals_",remove,".csv"))
  unlink(paste0("params",remove,".csv"))
  cbind(param2, simul2)
}
...
.ABC_rejection_internal_cluster <- function(model, prior, prior_test, nb_simul, seed_count = 0, 
  ...
  cl <- makeCluster(nodeslist, type = "PSOCK",outfile="errors.txt") 
...
.ABC_Lenormand_cluster <- function(model, prior, prior_test, nb_simul, summary_stat_target, 
  ...
  while (p_acc > p_acc_min) {
    ...
    chosen = paste0("sim_final",simul_below_tol[,ncol(simul_below_tol)],".csv")
    all = list.files(pattern = "sim_final")
    remove = all[which(!all%in% chosen)]
    unlink(remove)
    
    chosen = paste0("residuals_",simul_below_tol[,ncol(simul_below_tol)],".csv")
    all = list.files(pattern = "residuals_")
    remove = all[which(!all%in% chosen)]
    unlink(remove)
    
    chosen = paste0("params",simul_below_tol[,ncol(simul_below_tol)],".csv")
    all = list.files(pattern = "params")
    remove = all[which(!all%in% chosen)]
    unlink(remove)
